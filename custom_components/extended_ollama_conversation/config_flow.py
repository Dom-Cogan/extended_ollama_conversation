...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> None:
    ...
    if base_url == DEFAULT_CONF_BASE_URL:
        # Do not set base_url if using OllamAi for case of Ollama's base_url change
        base_url = None
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for OllamaAI Conversation."""
...
class OptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        schema = self.ollamaai_config_option_schema(self.config_entry.options)
        return self.async_show_form(
            step_id="init",
            data_schema=vol.Schema(schema),
        )

    def ollamaai_config_option_schema(self, options: MappingProxyType[str, Any]) -> dict:
